...
func (c *Client) DeleteCart(ctx context.Context, cartId int64) error {
	...
	resp, err := c.client.Do(req)
	if err != nil {
		c.logger.Error("Failed to initiate delete cart request", zap.Error(err), zap.Int64("cartID", cartId))
	...
	if resp.StatusCode >= 400 {
		c.logger.Error("Received an error from the server when deleting cart", zap.Int("status", resp.StatusCode))
...
func (c *Client) CreateCart(ctx context.Context) (*models.Cart, error) {
	...
	resp, err := c.client.Do(req)
	if err != nil {
		c.logger.Error("Failed to initiate create cart request", zap.Error(err))
	...
	if resp.StatusCode >= 400 {
		c.logger.Error("Received an error from the server when creating cart", zap.Int("status", resp.StatusCode))
	...
	body, err := io.ReadAll(resp.Body)
	if err != nil {
		c.logger.Error("Failed to read the response body from the cart creation response", zap.Error(err))
